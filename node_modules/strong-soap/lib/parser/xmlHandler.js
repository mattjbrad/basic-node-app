...
var XMLHandler = function () {
  ...
  _createClass(XMLHandler, [{
    key: 'jsonToXml',
    value: function jsonToXml(node, nsContext, descriptor, val) {
      if (node == null) {
      ...
      var name;
      var nameSpaceContextCreated = false;
      if (descriptor instanceof AttributeDescriptor) {
        name = descriptor.qname.name;
        if (descriptor.form === 'unqualified') {
          node.attribute(name, val);
        } else if (descriptor.qname) {
          var mapping = declareNamespace(nsContext, node, descriptor.qname.prefix, descriptor.qname.nsURI);
          var prefix = mapping ? mapping.prefix : descriptor.qname.prefix;
          var attrName = prefix ? prefix + ':' + name : name;
      ...
      if (descriptor instanceof ElementDescriptor) {
        name = descriptor.qname.name;
        var isSimple = descriptor.isSimple;
        var attrs = null;
        ...
        if (descriptor.form === 'unqualified') {
          ...
          var _mapping = nsContext.getPrefixMapping(descriptor.qname.nsURI);
          var newlyDeclared = false;
          // if namespace not declared, declare it
          if (_mapping === null || _mapping.declared === false) {
            newlyDeclared = true;
            _mapping = declareNamespace(nsContext, null, descriptor.qname.prefix, descriptor.qname.nsURI);
          }
          // add the element to a parent node
          var _prefix = _mapping ? _mapping.prefix : descriptor.qname.prefix;
          elementName = _prefix ? _prefix + ':' + name : name;
          // if namespace is newly declared add the xmlns attribute
          if (newlyDeclared) {
            xmlns = _prefix ? 'xmlns:' + _prefix : 'xmlns';
          }
        }
        ...
        if (isSimple) {
          ...
          var _attrs = val[this.options.attributesKey];
          if ((typeof _attrs === 'undefined' ? 'undefined' : _typeof(_attrs)) === 'object') {
            for (var p in _attrs) {
              var child = _attrs[p];
                  ...
                  if (descriptor.refOriginal) {
                    if (descriptor.refOriginal.typeDescriptor) {
                      if (descriptor.refOriginal.typeDescriptor.inheritance) {
                        var extension = descriptor.refOriginal.typeDescriptor.inheritance[child.type];
                        if (extension) {
        ...
        if (val != null && ((typeof val === 'undefined' ? 'undefined' : _typeof(val)) !== 'object' || val instanceof Date)) {
    ...
    key: 'mapObject',
    value: function mapObject(node, nsContext, descriptor, val, attrs) {
      if (val == null) return node;
      if ((typeof val === 'undefined' ? 'undefined' : _typeof(val)) !== 'object' || val instanceof Date) {
        node.text(val);
        return node;
      }
      ...
      var elements = {},
          attributes = {};
      if (descriptor != null) {
        for (var i = 0, n = descriptor.elements.length; i < n; i++) {
          var elementDescriptor = descriptor.elements[i];
          var elementName = elementDescriptor.qname.name;
          elements[elementName] = elementDescriptor;
        }
      }

      if (descriptor != null) {
        for (var a in descriptor.attributes) {
          var attributeDescriptor = descriptor.attributes[a];
          var attributeName = attributeDescriptor.qname.name;
      ...
      if (!Array.isArray(val)) {
        for (var p in val) {
          ...
          var child = val[p];
          var childDescriptor = elements[p] || attributes[p];
    ...
    key: 'addAttributes',
    value: function addAttributes(node, nsContext, descriptor, val, attrs) {
      var elements = {},
          attributes = {};
      if (attrs != null && (typeof attrs === 'undefined' ? 'undefined' : _typeof(attrs)) === 'object') {
        for (var p in attrs) {
          var child = attrs[p];
          // if field is $xsiType add xsi:type attribute
          if (p === this.options.xsiTypeKey) {
            var xsiType = void 0;
            if ((typeof child === 'undefined' ? 'undefined' : _typeof(child)) === 'object' && typeof child.type !== 'undefined') {
            ...
            var mapping = declareNamespace(nsContext, node, xsiType.prefix, xsiType.nsURI);
            var prefix = mapping ? mapping.prefix : xsiType.prefix;
            node.attribute('xsi:type', prefix ? prefix + ':' + xsiType.name : xsiType.name);
            continue;
          }
          var childDescriptor = attributes[p];
          if (childDescriptor == null) {
    ...
    key: '_processText',
    value: function _processText(top, val) {
      ...
      if (top.object === undefined) {
        ...
        var value = top.object[this.options.valueKey];
        if (value !== undefined) {
    ...
    key: 'xmlToJson',
    value: function xmlToJson(nsContext, xml, descriptor) {
      var self = this;
      ...
      p.onopentag = function (node) {
        ...
        for (var a in attrs) {
          if (/^xmlns:|^xmlns$/.test(a)) {
            var prefix = a === 'xmlns' ? '' : a.substring(6);
        ...
        for (var _a in attrs) {
          if (/^xmlns:|^xmlns$/.test(_a)) continue;
          var qname = QName.parse(_a);
          var isXsiType = false;
          var xsiType = null;
          var xsiXmlns = null;
      ...
      p.onclosetag = function (nsName) {
        ...
        if (top.object !== null) {
          if (_typeof(top.object) === 'object' && elementName in top.object) {
            // The element exist already, let's create an array
            var val = top.object[elementName];
            if (Array.isArray(val)) {
    ...
    key: 'createSOAPEnvelopeDescriptor',
    value: function createSOAPEnvelopeDescriptor(prefix, nsURI, parameterDescriptor) {
      prefix = prefix || 'soap';
      nsURI = nsURI || 'http://schemas.xmlsoap.org/soap/envelope/';
    ...
    key: 'parseXml',
    value: function parseXml(root, xml, cb) {
      var parser = void 0;
      var stringMode = true;
      ...
      parser.onopentag = function (node) {
        ...
        var element = current.element(node.name);
        if (node.attributes) {
...
function getSoap12FaultErrorMessage(faultBody) {
  var errorMessage = null;
  var code = selectn('Code', faultBody) || selectn('Code', faultBody);
  if (code) {
